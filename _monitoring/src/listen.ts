...
// import { TxPvtReadWriteSet } from '@hyperledger/fabric-protos/lib/ledger/rwset';
...
export async function main(client: Client): Promise<void> {
    ...
    try {
        ...
        const blocks = await network.getBlockAndPrivateDataEvents({
        ...
        try {
            for await (const blockProto of blocks) {
                const blockProcessor = new BlockProcessor({
                    block: parseBlock(blockProto.getBlock() as any),
                    checkpointer,
                ...
                // console.log('pd -------------');
                // const pdMap = blockProto.getPrivateDataMapMap();
                // for (const [key, value] of pdMap.entries()) {
                //     const privateRWSet: TxPvtReadWriteSet = value;
                //     privateRWSet.getNsPvtRwsetList().forEach(e => {
                //         e.getCollectionPvtRwsetList().forEach(f => {
                //             const prqs =  ledger.rwset.kvrwset.KVRWSet.deserializeBinary(f.getRwset_asU8());
                //             console.log(`Key=${key}, Namespace=${e.getNamespace()}, collection=${f.getCollectionName()}`);
                //             console.log(prqs.toObject());

                //             console.log(prqs.getWritesList());
                //             prqs.getWritesList().forEach(writes => {
                //                 const s = new TextDecoder().decode(writes.getValue_asU8());
                //                 console.log(`KV:: ${writes.getKey()} ${s}`);
                //             });
                //         });
                //     });
                // }
...
class TransactionProcessor {
    ...
    async process(): Promise<void> {
        ...
        const creator = this.#transaction.getCreator();
        const creatorStr = td.decode(creator);
        const certStart = creatorStr.indexOf('-----');
        if (certStart>0) {
            // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-call
            const x509Cert = new crypto.X509Certificate(creatorStr.slice(certStart));
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
            console.log(x509Cert);
        } else {
            console.log('IDEMIX ID ');
        }


        // console.log(`${td.decode(creator)}`);
        writes.forEach(w => {
            console.log(`${w.key} ${td.decode(w.value)}`);
        });
